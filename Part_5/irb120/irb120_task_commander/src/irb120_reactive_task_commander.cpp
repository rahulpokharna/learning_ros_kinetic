...
int main(int argc, char** argv) {
    ...
    // Goal and offset values set here
    double offset = 0.08;
    double goalX = -0.35;
    double goalY = 0.3;
    ...
    g_q_vec_arm_Xd << 0, 0, 0, 0, 0, 0; //assumes arm starts @this pose; better would be  to subscribe to joint_states to get actual angles
    ...
    // set arrival time, not to be changed
    arrival_time = 5.0;
    nsteps = 100; //arbitrary; tune me

    // Loop to keep running until gear is at goal
    while (fabs(goalY - g_perceived_object_pose.pose.position.y) > 0.05 || fabs(goalX - g_perceived_object_pose.pose.position.x) > 0.05) {

        // Set flange to poin down, home above the part
        ...
        // once above the goal, then go to offset point according to ineq
        if (g_perceived_object_pose.pose.position.x > goalX){
            flange_origin <<g_perceived_object_pose.pose.position.x + offset, g_perceived_object_pose.pose.position.y, 0.005;
        }
        else {
            flange_origin <<g_perceived_object_pose.pose.position.x - offset, g_perceived_object_pose.pose.position.y, 0.005;
        }
        ...
        // Move the object to the appropriate spot now
        if (g_perceived_object_pose.pose.position.x > goalX){
            flange_origin <<goalX + offset, g_perceived_object_pose.pose.position.y, 0.001;
        }
        else {
            flange_origin <<goalX - offset, g_perceived_object_pose.pose.position.y, 0.001;
        }
        ...
        // Re-find the object and updated the values for position
        ...
        // After finding the part again, re-home above the object
        ...
        // After rehoming, move down to offset pos for moving along the y
        
        if (g_perceived_object_pose.pose.position.y > goalY) {
            flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + offset, 0.001;
        }
        else {
            flange_origin <<g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - offset, 0.001;
        }
        ...
        // Now at offset pos, move part to the goal along the y axis
        
        if (g_perceived_object_pose.pose.position.y > goalY) {
            flange_origin <<g_perceived_object_pose.pose.position.x, goalY + offset, 0.001;
        }
        else {
            flange_origin <<g_perceived_object_pose.pose.position.x, goalY - offset, 0.001;
        }
        ...
        // Find the object again, to checkl the loop condition
        ...
        // move upward and rehome the part
